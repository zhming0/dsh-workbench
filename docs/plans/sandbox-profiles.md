...
- A Buildkite profile is
  `{ backend: buildkite, organization, pipeline, hostUrl, [image, readyTimeoutMs, tokenEnv] }`.
...
- A build cannot pause. The backend reports `supportsHibernate: false`, and the
  manager treats that capability as "checkpoint instead": on idle it commits
  the working tree inside the sandbox, pulls the commits the remote lacks out
  as a Git bundle into the host's state directory, then cancels the build and
  keeps the record as hibernated with the checkpoint attached. The next prompt
  provisions a new build, sets it up as a new session, unpacks the bundle, and
  puts the session back on its branch with the checkpoint commit undone. A
  failed save keeps the build running so the idle timer retries. The
...
- Not done: telling the model its sandbox was replaced, so it re-runs
  environment setup it did by hand.
