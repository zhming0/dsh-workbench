...
markers, no longer mid-merge. The model is not told the sandbox was replaced.
...
restore fails, the next prompt retries it against the same checkpoint and only
ever undoes the commit it made, so a checkpoint branch that was removed from
the remote produces an error on every prompt until the session is released. A
